	...
	while True:
		...
		try:
			...
			if len(reddit_objects) == 0:
				log.debug(f"{name} fetched {api_request.ids[0][3:]} to {api_request.ids[-1][3:]}, got {len(reddit_objects)} objects, requeueing")
				queue.put_nowait(api_request)
			else:
				log.debug(f"{name} fetched {api_request.ids[0][3:]} to {api_request.ids[-1][3:]}, got {len(reddit_objects)} objects")
...
async def producer(queue, start_id):
	...
	while True:
		now = datetime.utcnow()
		log.debug(f"producer: {queue.qsize()} : {len(request_list)}")
		...
		if len(request_list):
			...
			if api_request.results is not None:
				...
				for api_object in api_request.results:
					if "remindme" in api_object.body.lower():
						log.info(f"https://www.reddit.com{api_object.permalink}")
		...
		if queue.qsize() < 5:
			if latest_datetime is None or latest_datetime < now - timedelta(seconds=20):
				...
				log.debug(f"Queueing from {ids[0][3:]} to {ids[-1][3:]}")
